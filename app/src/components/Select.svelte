<script lang="ts">
  ...
  export let options: { label: string; value: any }[] = Object.entries(optionsMap).map(([value, label]) => ({ value, label }));
  ...
  const doValidate = (): boolean => {
    ...
    const posibs = options.map(option => {
      return option.value;
    })

    if(!posibs.includes(value)) {
...
<div class="select" on:click={() => !readonly && (open = !open)}>
  ...
  <div class="arrow">▼</div>
  {#if open}
    <div class="options elev3" use:accommodate on:click|stopPropagation={() => {}}>
      <Menu>
        {#each options as option}
          <MenuItem on:pointerdown={() => (value = option.value)}>
            <slot item={option}>
              {option.label}
            </slot>
          </MenuItem>
